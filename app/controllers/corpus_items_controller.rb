class CorpusItemsController < ApplicationController
  ...
  def create
    @corpus_item = CorpusItem.new(params[:corpus_item])

    respond_to do |format|
      if @corpus_item.save
        format.html { redirect_to @corpus_item, notice: 'Corpus item was successfully created.' }
  ...
  def update
    ...
    respond_to do |format|
      if @corpus_item.update_attributes(params[:corpus_item])
        format.html { redirect_to @corpus_item, notice: 'Corpus item was successfully updated.' }
        format.json { head :ok }
      else
